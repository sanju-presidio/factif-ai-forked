...
export const getLatestScreenshot = async (source?: StreamingSource): Promise<string | null> => {
    try {
        if (source === 'chrome-puppeteer') {
            ...
            try {
                ...
                const containerStatus = await DockerCommands.checkContainerStatus(containerName);
                if (containerStatus.exists && containerStatus.running && containerStatus.id) {
                    const screenshotPath = `/tmp/screenshot_${Date.now()}.png`;
                    const screenshot = await DockerCommands.takeScreenshot(containerStatus.id, screenshotPath);
                    return screenshot;
                }
            } catch (error) {
...
export const saveScreenshot = async (screenshot: string, folderPath: string, chatId: string): Promise<string> => {
    try {
