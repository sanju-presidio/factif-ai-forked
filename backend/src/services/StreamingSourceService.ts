...
import { StreamingService, StreamingSource, ServiceConfig } from '../types/stream.types';
...
class StreamingSourceService {
  ...
  constructor(io: SocketServer) {
    this.serviceConfig = { io };
    this.service = new PuppeteerService(this.serviceConfig); // Default to Puppeteer
  }

  private getService(source: StreamingSource): StreamingService {
    switch (source) {
      case 'ubuntu-docker-vnc':
        return new DockerVNCService(this.serviceConfig);
      case 'chrome-puppeteer':
        return new PuppeteerService(this.serviceConfig);
      default:
        throw new Error(`Unsupported streaming source: ${source}`);
    }
  }

  async initialize(url: string, source: StreamingSource): Promise<ActionResponse> {
