...
export class DockerVNCService extends BaseStreamingService {
  ...
  async initialize(url: string): Promise<ActionResponse> {
    ...
    try {
      const containerStatus = await DockerCommands.checkContainerStatus(this.config.containerName);
      
      if (containerStatus.exists && containerStatus.id) {
        this.containerId = containerStatus.id;
        
        if (!containerStatus.running) {
          this.emitConsoleLog('info', `Starting existing container ${this.containerId}...`);
          await DockerCommands.startContainer(this.containerId);
        } else {
          this.emitConsoleLog('info', `Using running container ${this.containerId}`);
        }
        ...
        this.emitConsoleLog('info', 'Connected to existing Ubuntu Docker VNC container');
        return {
          status: 'success',
      ...
      this.emitConsoleLog('error', `VNC initialization error: ${error.message || 'Unknown error'}`);
      await this.cleanup();
      throw error;
    }
  }
  ...
  async takeScreenshot(): Promise<string | null> {
    ...
    try {
      return await DockerCommands.takeScreenshot(this.containerId, screenshotPath);
    } catch (error: any) {
      this.emitConsoleLog('error', `Screenshot error: ${error.message || 'Unknown error'}`);
      return null;
    }
  }
  ...
  async performAction(action: string, params?: any): Promise<ActionResponse> {
    ...
    try {
      ...
      return await DockerActions.performAction(this.containerId, action, params);
    } catch (error: any) {
      this.emitConsoleLog('error', `VNC action error: ${error.message || 'Unknown error'}`);
      return {
        status: 'error',
  ...
  async cleanup(): Promise<void> {
    ...
    this.emitConsoleLog('info', 'VNC resources cleaned up, container left running');
  }
}
