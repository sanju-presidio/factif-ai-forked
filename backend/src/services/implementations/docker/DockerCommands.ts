...
export class DockerCommands {
  static async executeCommand({ command, successMessage, errorMessage }: DockerCommandOptions): Promise<string> {
  ...
  static async checkContainerStatus(containerName: string): Promise<ContainerStatus> {
    const command = [
      'ps',
      '-a',
  ...
  static async startContainer(containerId: string): Promise<void> {
    ...
    try {
      ...
      await this.executeCommand({
        command: ['exec', '-d', '-e', 'DISPLAY=:99', containerId, 'x11vnc', '-display', ':99', '-forever', '-shared', '-rfbport', '5900', '-nopw', '-xkb'],
      ...
      // Start noVNC
      await this.executeCommand({
        command: ['exec', '-d', containerId, 'websockify', '--web', '/opt/noVNC', '6080', 'localhost:5900'],
  ...
  static async takeScreenshot(containerId: string, screenshotPath: string): Promise<string> {
    const tempDir = os.tmpdir();
    const tempScreenshotPath = path.join(tempDir, `screenshot-${Date.now()}.png`);
    ...
    try {
      ...
      try {
        ...
        await this.executeCommand({
          command: ['exec', containerId, 'gnome-screenshot', '-f', screenshotPath],
          errorMessage: 'Screenshot failed'
        });
      }
      ...
      const imageBuffer = await sharp(tempScreenshotPath)
        .jpeg()
        .toBuffer();
