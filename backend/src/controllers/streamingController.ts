...
export class StreamingController {
    ...
    async handleStartStream(socket: Socket, { url, source = 'chrome-puppeteer', folderPath }: { url: string, source?: StreamingSource, folderPath?: string }) {
        try {
            ...
            socket.emit('stream-error', { message: error?.message || 'Unknown error occurred' });
        }
    }

    async handleBrowserAction(socket: Socket, { action, params, folderPath }: { action: string, params: any, folderPath?: string }) {
        try {
            const paramsWithPath = { ...params, folderPath };
            await this.streamingService.performAction(action, paramsWithPath);
        } catch (error: any) {
            socket.emit('browser-error', { message: error?.message || 'Unknown error occurred' });
        }
    }

    async handleRequestScreenshot(socket: Socket, { folderPath }: { folderPath?: string }) {
        try {
    ...
    async handleStopBrowser(socket: Socket, { folderPath }: { folderPath?: string }) {
        try {
