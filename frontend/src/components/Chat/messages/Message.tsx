    ...
    case 'followup_question':
      return (
        <div className="my-2 pl-2 border-l-4 border-warning bg-warning/10 p-2 rounded">
          ...
          <ReactMarkdown components={markdownComponents}>{part.question}</ReactMarkdown>
        </div>
      );
    ...
    case 'complete_task':
      return (
        <div className="mb-2 text-success pt-3 rounded">
          ...
          <ReactMarkdown components={markdownComponents}>{part.result}</ReactMarkdown>
          {part.command && (
            <div className="mt-2 bg-content1 p-2 rounded">
              <code className="text-sm font-mono text-success">{part.command}</code>
            </div>
          )}
        </div>
      );
      ...
      return (
        <div className="mt-2 bg-content1 rounded-lg p-3 border border-border">
          <div className="space-y-2">
            <div>
              <span className="text-foreground/60 text-sm">Action: <span className="text-warning font-medium">{part.action}</span></span>
            </div>
            {part.url && (
              <div>
                <span className="text-foreground/60 text-sm">URL: <span className="text-foreground font-mono">{part.url}</span></span>
              </div>
            )}
            {part.coordinate && (
              <div>
                <span className="text-foreground/60 text-sm">Coordinate: <span className="text-foreground font-mono">{part.coordinate}</span></span>
              </div>
            )}
            {part.text && (
              <div>
                <span className="text-foreground/60 text-sm">Text: <span className="text-foreground font-mono">{part.text}</span></span>
              </div>
            )}
            {part.key && (
              <div>
                <span className="text-foreground/60 text-sm">Key: <span className="text-foreground font-mono">{part.key}</span></span>
              </div>
            )}
          </div>
          </div>
          ...
          {part.screenshot && (
            <div className="mt-3">
              <div className="bg-content1 rounded-lg border border-border overflow-hidden">
                ...
                {showProcessed && part.omniParserResult && part.omniParserResult.parsed_content && (
                  <div className="border-t border-border">
                    ...
                    >
                      <span className="text-sm text-foreground/60">
                        {part.omniParserResult.parsed_content.length} elements detected
                      </span>
                      <svg 
                    ...
                    {showElements && (
                      <div className="p-3 space-y-1 border-t border-border">
                        {part.omniParserResult.parsed_content.map((element, idx) => (
...
export const Message = ({ text, isUser, isPartial }: MessageProps) => {
  ...
  return (
    <div className="flex justify-start animate-fade-in">
      <div className={`max-w-[90%] text-white text-left ${isPartial ? "animate-pulse" : ""}`}>
