...
export class MessageProcessor {
  ...
  static parseActionResult(text: string): string {
    const match = text.match(/<perform_action_result>[\s\S]*?<action_message>(.*?)<\/action_message>[\s\S]*?<\/perform_action_result>/);
    return match ? match[1] : text;
  }

  static async processMessage(chunk: string, source: StreamingSource): Promise<ProcessedMessage> {
    ...
    if (action) {
      try {
        ...
        const actionMessage = response.message || response.error || 'Action completed';
...
${response.omniParserResult ? `<omni_parser>${JSON.stringify(response.omniParserResult)}</omni_parser>` : ''}
        ...
        const omniParserMatch = actionResult.match(/<omni_parser>(.*?)<\/omni_parser>/s);
        const omniParserResult = omniParserMatch ? JSON.parse(omniParserMatch[1]) : undefined;
